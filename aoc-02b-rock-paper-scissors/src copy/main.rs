...
fn main() {
    ...
    if let Ok(lines) = read_lines("./resources/input.txt") {
        ...
        for line in lines {
            if let Ok(ip) = line {
                let round = get_round_from_line(ip, plays.clone());
                player_points += round.player_points;
            }
        }
    }
...
fn get_round_from_line(line: String, plays_map: HashMap<&str, Play>) -> Round {
    ...
    let mut round = Round { opponent: opponent_play, player: player_play, player_points: 0 };

    match winner_number {
        0 => {
            round.player_points = play_points + 3;
        }
        1 => {
            round.player_points = play_points;
        }
        2 => {
            round.player_points = play_points + 6;
        }
        _ => {
            panic!("Match error")
        }
