...
fn main() {
    ...
    if let Ok(lines) = read_lines("./resources/input.txt") {
        ...
        for line in lines {
            if let Ok(ip) = line {
                let player_round_points = get_round_points_from_line(ip, plays.clone());
                player_points += player_round_points;
            }
        }
    }

    println!("Player points: {player_points}"); //13509
}
...
fn get_losing_play(play: Play) -> Play {
    match play {
...
fn get_drawing_play(play: Play) -> Play {
    play
}
...
fn get_round_points_from_line(line: String, plays_map: HashMap<&str, Play>) -> u16 {
    ...
    let result_char = line_chars.nth(1).unwrap().to_string();

    let opponent_play = plays_map[&opponent_char.as_str()];



    let player_play = match result_char.as_str() {
        "X" => get_losing_play(opponent_play),
        "Y" => get_drawing_play(opponent_play),
        "Z" => get_winning_play(opponent_play),
        _ => panic!("Error while matching")
    };
